...
Docker is used to containerize apps thus giving apps an isolated ENV which is not machine-specific. `Think docker image as CLASS & docker containers as OBJECTS/Runtime Instance` OR `Think image (as OS) which can be used by many containers (Machines)`.
...
3. `--network` (Connects container to a specific network.) | `-p` Maps host port → container port (host: container) 
